/**
 * @file   secureRoom.cpp
 * @brief  Implementation file for the Secure_Room class.
 *
 * This file contains the grantAccess method, which is specific
 * to accessing a Secure Room. Each room type has different permissions based on the
 * time and role of the CardHolder attempting to access them. Modifications to the
 * grantAccess method will directly impact access to Secure Room.
 *
 * Additionally, the file includes the implementation of the saveToFile method,
 * responsible for saving Secure_Room data. This functionality is accessed by
...
#include <ctime>
 ...
 /**
  * @brief Determines whether access is granted to the Secure_Room based on the CardHolder's role and current time.
  *
...
bool Secure_Room::grantAccess(const CardHolder& cardHolder) const {
 ...
 *
 * This method writes the specific data members of the Secure_Room object, including its type,
...
void Secure_Room::saveToFile(ofstream& outFile) const {
    //implement the saving logic specific to Secure_Room
    //write the data members of Secure_Room to the file
    outFile << "Secure_Room " << getBuildingCode() << " " << getFloor() << " " << getRoomNumber() << "\n";
}